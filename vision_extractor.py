...
class VisionExtractor:
    ...
    def setup_ui(self):
        ...
        self.img_label = tk.Label(self.frame, text="Image will appear here", font=("Arial", 12))
        self.img_label.pack(pady=10)

        # Text field to display extracted text
        self.text_label = tk.Label(self.frame, text="Extracted CAPTCHA text will appear here.", font=("Arial", 14))
        self.text_label.pack(pady=10)

        # Button to select and process image
        self.select_button = tk.Button(self.frame, text="Select CAPTCHA Image", command=self.select_and_process_image)
        self.select_button.pack(pady=10)

        # Button to navigate back to the Home screen
        if self.show_frame:
            tk.Button(self.frame, text="Back to Home", command=lambda: self.show_frame("Home")).pack(pady=10)
